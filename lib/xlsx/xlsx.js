...
class XLSX {
    ...
    model.sheets.forEach((sheet) => {
      sheet.rows.forEach((row) => {
        row.cells.forEach((cell) => {
          ...
          formulae.push({ address: cell.address, i: sheet.id });
        });
      });
    });
    
    zip.append(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><calcChain xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">${formulae.map(formula => `<c r="${formula.address}" i="${formula.i}" />`).join('')}</calcChain>`, {name: 'xl/calcChain.xml'});
  }
  ...
  async write(stream, options) {
    ...
    await this.addCalcChain(zip, model);
